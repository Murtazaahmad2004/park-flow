import { FaBookmark, FaCalendarCheck, FaCar, FaCheck, FaCreditCard, FaHistory, FaParking, FaSignOutAlt } from 'react-icons/fa'; // icons import kar rahi hai.
...
import { NavLink } from 'react-router-dom'; // Pages ke darmiyan navigation.
import { motion } from "framer-motion"; // Elements ko animate karta hai.
import { LineChart, XAxis, YAxis, Line, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell} from 'recharts' // Dashboard graphs banane ke liye.
...
<div className="user-home-header">
  <div className="user-home-nav-bar">
      <div className="user-home-logo">
               <NavLink to="#" className="user-home-logo-link" onClick={scrollToTop}>
                  <div className="user-home-logo">
...
>
      <h3>Parking Duration</h3>
         <LineChart width={890} height={350} data={linedata}>
            ...
            <Line type="monotone" dataKey="booked" stroke="#185FA5" strokeWidth={2} dot={{ r: 4 }} /> {/* monotype smooth curved line banata hai. */}
            <Line type="monotone" dataKey="available" stroke="#3ecf8e" strokeWidth={2} dot={{ r: 4 }} />
         </LineChart>
</motion.div>
...
>
   <h3 className="chart-title">Parking Overview</h3>
         <ResponsiveContainer width="100%" height={290}> {/* Responsive Container Different screen sizes pe adjust hota ha */}
            <PieChart> {/* pie chart ka main container */}
               <Pie
                  ...
                  dataKey="value"
                  >
                  {donutdata.map((curitem, i) => ( // har data items k liya cells create ho rahy han (curitem(current item) (i(indexing k liya ha color assign k liya)
