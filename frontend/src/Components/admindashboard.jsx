import { FaFileInvoiceDollar, FaParking, FaSignOutAlt, FaUserPlus, FaUserTie, FaCar, FaBookmark } from 'react-icons/fa';
import { LineChart, BarChart, XAxis, YAxis, Line, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, Bar} from 'recharts' // Dashboard graphs banane ke liye.
...
useEffect(() => {
document.title = "Dashboard - ParkFlow";
});
return (
<>
...
<div className="main-dashboard">
   ...
   <motion.div
      ...
      <LineChart width={900} height={350} data={revenuedata}>
         ...
         <Line type="monotone" dataKey="month" stroke="#185FA5" strokeWidth={2} dot={{ r: 4 }} /> 
         <Line type="monotone" dataKey="profit" stroke="#0fa80a" strokeWidth={2} dot={{ r: 4 }} />
   ...
   <ResponsiveContainer width="100%" height={290}>
      ...
      <PieChart>
         ...
         label
         >
         {donutdata.map((curitem, i) => ( // har data items k liya cells create ho rahy han (curitem (current item han) aur i (index) han)
   ...
   {/* start vehicals entries bar graph */}
      ...
      <BarChart width={900} height={350} data={vehicalentries}>
         ...
         <Legend />
         {/* Chart labels show karta hai */}
         <Bar dataKey="vehicals" fill="#4472a0" barSize={30} name="Vehicals Entries"/> {/* monotone smooth curved line banata hai. */}
      </BarChart>
   </motion.div>
   ...
   <ResponsiveContainer width="100%" height={290}>
      ...
      <PieChart>
         ...
         label
         >
         {vehicaltype.map((curitem, i) => ( // har data items k liya cells create ho rahy han (curitem (current item han) aur i (index) han)
         <Cell key={i} fill={colors[i]} /> // Har pie section ka color set kar raha hai. (key unique index hai, fill colors array se color le raha hai)
         ))}
         </Pie>
         <Tooltip />
